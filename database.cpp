...
namespace Vlinder { namespace RTIMDB {
	...
	unsigned int Database::createPoint(Details::Producer const *producer, Core::PointType const &point_type, Core::PointValue const &initial_value/* = Core::PointValue()*/, Core::Flags const &initial_flags/* = Core::Flags()*/, Core::Timestamp const &initial_timestamp/* = Core::Timestamp()*/)		{ auto result(createPoint(producer, point_type, initial_value, initial_flags, initial_timestamp, nothrow)); throwException(result.second); return result.first; }
	...
	std::pair< Core::Details::Optional< Core::Point >, Errors > Database::read(Core::Details::ROTransaction const &transaction, Core::PointType point_type, unsigned int system_id) const noexcept
	{
		return data_store_.read(transaction, point_type, system_id RTIMDB_NOTHROW_ARG);
	}

	std::pair< unsigned int, Errors > Database::createPoint(Details::Producer const *producer, Core::PointType const &point_type RTIMDB_NOTHROW_PARAM) noexcept
	{
		return createPoint(producer, point_type, Core::PointValue(), Core::Flags(), Core::Timestamp() RTIMDB_NOTHROW_ARG);
	}
	std::pair< unsigned int, Errors > Database::createPoint(Details::Producer const *producer, Core::PointType const &point_type, Core::PointValue const &initial_value RTIMDB_NOTHROW_PARAM) noexcept
	{
		return createPoint(producer, point_type, initial_value, Core::Flags(), Core::Timestamp() RTIMDB_NOTHROW_ARG);
	}
	std::pair< unsigned int, Errors > Database::createPoint(Details::Producer const *producer, Core::PointType const &point_type, Core::PointValue const &initial_value, Core::Flags const &initial_flags RTIMDB_NOTHROW_PARAM) noexcept
	{
		return createPoint(producer, point_type, initial_value, initial_flags, Core::Timestamp() RTIMDB_NOTHROW_ARG);
	}
	std::pair< unsigned int, Errors > Database::createPoint(Details::Producer const *producer, Core::PointType const &point_type, Core::PointValue const &initial_value, Core::Flags const &initial_flags, Core::Timestamp const &initial_timestamp RTIMDB_NOTHROW_PARAM) noexcept
	{
		pre_condition((producer >= producers_) && (producer < producers_ + (sizeof(producers_) / sizeof(producers_[0]))));
		Core::PointValue point(initial_value);
		auto result(data_store_.insert(point_type, initial_value, initial_flags, initial_timestamp RTIMDB_NOTHROW_ARG));
	...
	Details::TransitionQueueTransaction Database::beginTransaction(Details::Producer const *producer, Core::Timestamp const &timestamp) noexcept
	{
	...
	Errors Database::update(RTIMDB_NOTHROW_PARAM_1) noexcept
	{
		...
		for (auto transition_queue(begin(transition_queues_)); transition_queue != end(transition_queues_); ++transition_queue)
		{
			...
			for (auto transition_count(transition_queue->size()); (retval == Errors::no_error__) && transition_count; --transition_count)
			{
				...
				{
					latest_timestamp_[transition_queue - transition_queues_] = transition.first.get< Core::Timestamp >();
					if (!first)
					{
				...
				else
				{
					...
					Errors update_result(data_store_.update(transaction.first, value.point_type_, value.system_id_, value.point_value_, value.flags_, latest_timestamp_[transition_queue - transition_queues_] RTIMDB_NOTHROW_ARG));
	...
	Errors Database::commitTransaction(Core::Timestamp const &timestamp, Core::Details::Transaction &transaction) noexcept
	{
		...
		for_each(
			...
			, [&](decltype(*(transaction.begin())) entry){
				...
				{
					commit_result = dispatch(timestamp, entry.type_, entry.point_id_, entry.value_.get(), entry.flags_.get());
				}
				else
	...
	Errors Database::dispatch(
		  Core::Timestamp const &timestamp
		, Core::PointType type
		, unsigned int id
		, Core::PointValue const &value
		, Core::Flags const &flags
		) noexcept
	{
		Errors retval(Errors::no_error__);
		for_each(
			...
			, [&](remove_reference< decltype(*begin(consumers_)) >::type &consumer) {
				if (Errors::no_error__ == retval)
				{
					...
					{
						retval = consumer.event_queues_[static_cast< unsigned int >(mapping->event_class_) - 1].push(Details::Event(timestamp, type, id, value, flags, mapping->tag_) RTIMDB_NOTHROW_ARG);
					}
					else
